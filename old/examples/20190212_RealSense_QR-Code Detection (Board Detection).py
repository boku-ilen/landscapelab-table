...
# Open a video with a QR code, find the QR code on each frame, and, if successful,
# show the code corners and un-warp the code to get a canonical view:
cap = cv2.VideoCapture('VID_20190212_163450.mp4')
try:
    while True:
        ...
        result, corners = qr_code_outer_corners(color_image)
        qr_code_size = 300
        if result:
            if all((0, 0) < tuple(c) < (color_image.shape[1], color_image.shape[0]) for c in corners):
                rectified = rectify(color_image, corners, (qr_code_size, qr_code_size))
                cv2.circle(color_image, tuple(corners[0]), 15, (0, 255, 0), 2)
                cv2.circle(color_image, tuple(corners[1]), 15, (0, 0, 255), 2)
                cv2.circle(color_image, tuple(corners[2]), 15, (255, 0, 0), 2)
                cv2.circle(color_image, tuple(corners[3]), 15, (255, 255, 0), 2)
                color_image[0:qr_code_size, 0:qr_code_size] = rectified
